...
def WriteJson(PTime):
  con = config.config
  base = 'https://rest.coinapi.io/v1/ohlcv/HUOBIPRO_SPOT_EOS_USDT/history?'
  period = PTime
  limit = '50'
...
def WriteSQL(time):
  ...
  for i in range(len(json_str)):
    ...
    if time == '1D':
      sql = 'INSERT INTO HUOBI_EOS_1d(exchange,start_at,end_at,open,high,low,close,volume,trades_count,interval_at)values("HUOBI",%s,%s,%s,%s,%s,%s,%s,%s,"1D")'
    elif time == '1H':
      sql = 'INSERT INTO HUOBI_EOS_1h(exchange,start_at,end_at,open,high,low,close,volume,trades_count,interval_at)values("HUOBI",%s,%s,%s,%s,%s,%s,%s,%s,"1H")'
